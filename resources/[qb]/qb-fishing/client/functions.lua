...
TryToFish = function()
    QBCore.Functions.TriggerCallback('qb-fishing:GetItemData', function(count)
        ...
        if count ~= nil then
            if count == 0 then
                QBCore.Functions.Notify("Du skal bruge en fiskestang og madding.", "primary")
            else
                ...
                if waterValidated then
                    ...
                    QBCore.Functions.Notify("Du skal sigte efter vandet for at fiske...", "primary")
                end
            end
        end
...
CastBait = function(rodHandle, castLocation)
    ...
    if not HasFishingBait() then
        QBCore.Functions.Notify('Du har ingen fangst!', 'error')
    ...
    while not IsControlJustPressed(0, 47) do
        ...
        ShowHelpNotification("Kast linen ud på ~INPUT_DETONATE~")

        if GetGameTimer() - startedCasting > 5000 then
            QBCore.Functions.Notify("Du skal kaste maddingen.", "primary")
    ...
    QBCore.Functions.Notify("Venter på bid...", "success", "10000")
...
TryToCatchFish = function()
    ...
    while true do
        ...
        ShowHelpNotification("Tryk ~INPUT_CONTEXT~ i det grønne område.")
...
IsInWater = function()
    ...
    QBCore.Functions.Notify("Tjekker fiskelokation...", "success", "3000")
