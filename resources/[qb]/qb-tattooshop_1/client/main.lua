...
Citizen.CreateThread(function()
	...
	for k, v in pairs(Config.Shops) do
		...
		BeginTextCommandSetBlipName("Tatovør")
...
function RemoveTattoo(name, label)
	...
	QBCore.Functions.Notify("Du fjernede: " .. GetLabelText(label) .. " tatoveringen", 'success')
end

function CreateScale(sType)
	if scaleString ~= sType and sType == "OpenShop" then
		scaleType = setupScaleform("instructional_buttons", "Åbn katalog", 38)
		scaleString = sType
	elseif scaleString ~= sType and sType == "Control" then
		scaleType = setupScaleform2("instructional_buttons", "Ændre kamera vinkel", 21, "Størrelse", {90, 89}, "Køb/Fjern tatovering", 191)
		scaleString = sType
	end
end

Citizen.CreateThread(function()
	TattooMenu.CreateMenu("tattoo", "Tatovør", function()
        return CloseTattooShop()
    end)
    TattooMenu.SetSubTitle('tattoo', "Kategorier")
						...
						if GetEntityModel(PlayerPedId()) == `mp_m_freemode_01` then
							if tattoo.HashNameMale ~= '' then
								...
								if found then
									...
									local clicked, hovered = TattooMenu.Button(GetLabelText(tattoo.Name), "~HUD_COLOUR_GREENDARK~DKK" .. price)
									if clicked then
