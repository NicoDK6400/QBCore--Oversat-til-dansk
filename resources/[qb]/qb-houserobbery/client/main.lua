...
local function PoliceCall()
    ...
    if math.random(1, 100) <= chance then
        TriggerServerEvent('police:server:policeAlert', 'Forsøg på indbrud')
    end
end
        ...
        if usingAdvanced then
            if closestHouse ~= nil then
                if CurrentCops >= Config.MinimumHouseRobberyPolice then
                    ...
                    QBCore.Functions.Notify('Ikke nok betjente..', 'error', 3500)
                end
            end
        else
            QBCore.Functions.TriggerCallback('QBCore:HasItem', function(result)
                if closestHouse ~= nil then
                    if result then
                        ...
                        QBCore.Functions.Notify('Ser ud til at du mangler noget...', 'error', 3500)
                    end
                end
            end, "screwdriverset")
        end
                        ...
                        if dist <= 1.5 then
                            ...
                            if CurrentCops >= Config.MinimumHouseRobberyPolice then
                                if Config.Houses[k]["opened"] then
                                    DrawText3Ds(Config.Houses[k]["coords"]["x"], Config.Houses[k]["coords"]["y"], Config.Houses[k]["coords"]["z"], '~g~E~w~ - For at tilgå')
