...
function QBCore.Player.CreatePlayer(PlayerData)
	...
	self.Functions.AddItem = function(item, amount, slot, info)
		...
		if itemInfo == nil then TriggerClientEvent('QBCore:Notify', self.PlayerData.source, 'Item eksistere ikke', 'error') return end
		local amount = tonumber(amount)
...
function QBCore.Player.DeleteCharacter(source, citizenid)
	...
	if license == result then
		...
		TriggerEvent('qb-log:server:CreateLog', 'anticheat', 'Anti-Cheat', 'white', GetPlayerName(src)..' Er blevet banned for at prøve at exploit', false)
	end
end
