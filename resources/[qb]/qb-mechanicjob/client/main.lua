...
local function RepairPart(part)
    ...
    QBCore.Functions.TriggerCallback('qb-inventory:server:GetStashItems', function(StashItems)
        ...
        if hasitem and countitem >= PartData.costs then
            ...
            QBCore.Functions.Progressbar("repair_part", "Reperere..." ..Config.ValuesLabels[part], math.random(5000, 10000), false, true, {
            ...
            QBCore.Functions.Notify('Der er ikke nok materiale!', 'error')
        end
    end, "mechanicstash")
end
...
RegisterNetEvent('qb-vehicletuning:client:RepaireeePart', function(part)
    ...
    QBCore.Functions.Notify(""..Config.ValuesLabels[part].." er blevet repereret!")
end)
        ...
        if veh ~= nil and veh ~= 0 then
            ...
            if #(pos - vehpos) < 5.0 then
                if not IsThisModelABicycle(GetEntityModel(veh)) then
                    ...
                    QBCore.Functions.Notify("Ikke en gyldigt køretøj", "error")
                end
            else
                QBCore.Functions.Notify("Du er ikke tæt nok på køretøjet", "error")
            end
        else
            QBCore.Functions.Notify("Du skal være i et køretøj", "error")
        end
    else
        QBCore.Functions.Notify("Du skal forlade køretøjet!", "error")
    end
end)
...
RegisterNetEvent('vehiclemod:client:fixEverything', function()
    if (IsPedInAnyVehicle(PlayerPedId(), false)) then
        ...
        QBCore.Functions.Notify("Du er ikke i/på køretøjet", "error")
    end
end)
...
RegisterNetEvent('vehiclemod:client:repairPart', function(part, level, needAmount)
    if not IsPedInAnyVehicle(PlayerPedId(), false) then
        ...
        if veh ~= nil and veh ~= 0 then
                ...
                if not IsThisModelABicycle(GetEntityModel(veh)) then
                    ...
                    if VehicleStatus[plate] ~= nil and VehicleStatus[plate][part] ~= nil then
                        ...
                        QBCore.Functions.Progressbar("repair_advanced", "Repair Vehicle", lockpickTime, false, true, {
                            ...
                            openingDoor = false
                            ClearPedTasks(PlayerPedId())
                            QBCore.Functions.Notify("Processen afbrudt", "error")
                        end)
                    else
            ...
            QBCore.Functions.Notify("Du skal være på/i køretøjet", "error")
        end
    else
        QBCore.Functions.Notify("Du er ikke i/på køretøjet", "error")
    end
...
CreateThread(function()
    ...
    AddTextComponentSubstringPlayerName("Autocare Mekaniker")
            ...
            if PlayerJob.name == "mechanic" then
                ...
                if onDuty then
                    if StashDistance < 20 then
                        ...
                        if StashDistance < 1 then
                            DrawText3Ds(Config.Locations["stash"].x, Config.Locations["stash"].y, Config.Locations["stash"].z, "[E] Åbn stash")
                ...
                if OnDutyDistance < 20 then
                    ...
                    if OnDutyDistance < 1 then
                        if onDuty then
                            DrawText3Ds(Config.Locations["duty"].x, Config.Locations["duty"].y, Config.Locations["duty"].z, "[E] Af arbejde")
                        else
                            DrawText3Ds(Config.Locations["duty"].x, Config.Locations["duty"].y, Config.Locations["duty"].z, "[E] På arbejde")
                        end
                        if IsControlJustReleased(0, 38) then
                            TriggerServerEvent("QBCore:ToggleDuty")
                        end
                    ...
                    for k, v in pairs(Config.Plates) do
                        if v.AttachedVehicle == nil then
                            ...
                            if PlateDistance < 20 then
                                ...
                                if PlateDistance < 2 then
                                    ...
                                    if IsPedInAnyVehicle(PlayerPedId()) then
                                        if not IsThisModelABicycle(GetEntityModel(veh)) then
                                            DrawText3Ds(v.coords.x, v.coords.y, v.coords.z + 0.3, "[E] Placér køretøj på platformen")
                                            if IsControlJustPressed(0, 38) then
                                                DoScreenFadeOut(150)
                                                Wait(150)
                            ...
                            if PlateDistance < 3 then
                                inRange = true
                                DrawText3Ds(v.coords.x, v.coords.y, v.coords.z, "[E] Åbn menu")
                                if IsControlJustPressed(0, 38) then
                                    OpenMenu()
                                end
                            end
