...
QBCore.Commands.Add("addgarage", "Tilføj garage (Kun ejendomsmæglere)", {}, false, function(source)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "realestate" then
...
RegisterNetEvent('qb-houses:server:removeHouseKey', function(house, citizenData)
    ...
    TriggerClientEvent('QBCore:Notify', src, 'Nøglerne er blevet taget fra ' .. citizenData.firstname .. ' ' .. citizenData.lastname, 'error')
...
RegisterNetEvent('qb-houses:server:giveHouseKey', function(target, house)
    ...
    if tPlayer then
        if housekeyholders[house] then
            for _, cid in pairs(housekeyholders[house]) do
                if cid == tPlayer.PlayerData.citizenid then
                    TriggerClientEvent('QBCore:Notify', src, 'Denne person har allerede nøglerne til boligen!', 'error', 3500)
                    return
                end
            end
        ...
        TriggerClientEvent('QBCore:Notify', src, 'Noget gik galt, prøv igen!', 'error', 2500)
    end
end)
