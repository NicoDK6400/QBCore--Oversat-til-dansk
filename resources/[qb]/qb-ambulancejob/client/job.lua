    ...
    QBCore.Functions.TriggerCallback('QBCore:HasItem', function(hasItem)
        if hasItem then
            ...
            if player ~= -1 and distance < 5.0 then
                ...
                QBCore.Functions.Progressbar("hospital_revive", "Genopliver personen...", 5000, false, true, {
    ...
    QBCore.Functions.TriggerCallback('QBCore:HasItem', function(hasItem)
        if hasItem then
            ...
            if player ~= -1 and distance < 5.0 then
                local playerId = GetPlayerServerId(player)
                isHealingPerson = true
                QBCore.Functions.Progressbar("hospital_healwounds", "Healer sår...", 5000, false, true, {
                    disableMovement = false,
                    disableCarMovement = false,
                    disableMouse = false,
                    disableCombat = true,
            ...
            if PlayerJob.name =="ambulance" then
                for k, v in pairs(Config.Locations["duty"]) do
                    ...
                    if dist < 5 then
                        ...
                        if dist < 1.5 then
                            if onDuty then
                                DrawText3D(v.x, v.y, v.z, "~r~E~w~ - Hold fyraften")
                            else
                ...
                for k, v in pairs(Config.Locations["armory"]) do
                    ...
                    if dist < 4.5 then
                        if onDuty then
                            if dist < 1.5 then
                                sleep = 0
                                DrawText3D(v.x, v.y, v.z, "~g~E~w~ - Lager")
                                if IsControlJustReleased(0, 38) then
                                    TriggerServerEvent("inventory:server:OpenInventory", "shop", "hospital", Config.Items)
                                end
                            elseif dist < 2.5 then
                                DrawText3D(v.x, v.y, v.z, "Lager")
                            end
                        end
                    end
                end
