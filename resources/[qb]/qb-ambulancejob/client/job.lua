    ...
    while true do
        ...
        if isLoggedIn and QBCore ~= nil then
                ...
                for k, v in pairs(Config.Locations["duty"]) do
                    ...
                    if dist < 5 then
                        if dist < 1.5 then
                            sleep = 7
                            if onDuty then
                                DrawText3D(v.x, v.y, v.z, "~r~E~w~ - Gå af arbejde")
                            else
                                DrawText3D(v.x, v.y, v.z, "~g~E~w~ - Gå på arbejde")
                            end
                        ...
                        elseif dist < 4.5 then
                            DrawText3D(v.x, v.y, v.z, "af/på duty")
                        end  
                    end
                end
                ...
                for k, v in pairs(Config.Locations["armory"]) do
                    ...
                    if dist < 4.5 then
                        if onDuty then
                            if dist < 1.5 then
                                sleep = 7
                                DrawText3D(v.x, v.y, v.z, "~g~E~w~ - Artikler")
                                if IsControlJustReleased(0, 38) then
                                    TriggerServerEvent("inventory:server:OpenInventory", "shop", "hospital", Config.Items)
                                end
                            elseif dist < 2.5 then
                                DrawText3D(v.x, v.y, v.z, "Shop")
                            end  
                        end
                    end
                end
                        ...
                        if onDuty then
                            ...
                            if dist < 1.5 then
                                if IsPedInAnyVehicle(ped, false) then
                                    ...
                                    DrawText3D(v.x, v.y, v.z, "~g~E~w~ - Helikopter")
                                end
                                if IsControlJustReleased(0, 38) then
                                    if IsPedInAnyVehicle(ped, false) then
                                        QBCore.Functions.DeleteVehicle(GetVehiclePedIsIn(ped))
                                        ...
                                        QBCore.Functions.SpawnVehicle(Config.Helicopter, function(veh)
                                            SetVehicleNumberPlateText(veh, "LIV"..tostring(math.random(1000, 9999)))
            ...
            for k, v in pairs(Config.Locations["main"]) do
                ...
                if dist < 1.5 then
                    sleep = 7
                    DrawText3D(v.x, v.y, v.z, "~g~E~w~ - Tag elevatoren op til taget")
                    if IsControlJustReleased(0, 38) then
                        DoScreenFadeOut(500)
...
AddEventHandler('hospital:client:RevivePlayer', function()
    QBCore.Functions.TriggerCallback('hospital:server:HasFirstAid', function(hasItem)
        if hasItem then
            ...
            QBCore.Functions.Notify("Du mangler et Første hjælps Kit", "error")
        end
    end, 'firstaid')
end)
...
AddEventHandler('hospital:client:TreatWounds', function()
    QBCore.Functions.TriggerCallback('hospital:server:HasBandage', function(hasItem)
        if hasItem then
            ...
            QBCore.Functions.Notify("Du mangler Bandage", "error")
        end
    end, 'bandage')
end)
