...
BodyParts = {
    ...
    ['LFINGER'] = { label = 'venstre fingre', causeLimp = false, isDamaged = false, severity = 0 },
...
WeaponDamageList = {
	["WEAPON_UNARMED"] = "Mærker fra næver",
	["WEAPON_ANIMAL"] = "Sår fra et bid (dyr)",
	["WEAPON_COUGAR"] = "Sår fra et bid (dyr)",
	["WEAPON_KNIFE"] = "Kniv stik",
	["WEAPON_NIGHTSTICK"] = "Slag fra en pind eller noget lignende",
	["WEAPON_BREAD"] = "Buler i hovedet af en baguette!",
	["WEAPON_HAMMER"] = "Slag fra en pind eller noget lignende",
	["WEAPON_BAT"] = "Slag fra en pind eller noget lignende",
	["WEAPON_GOLFCLUB"] = "Slag fra en pind eller noget lignende",
	["WEAPON_CROWBAR"] = "Slag fra en pind eller noget lignende",
        ...
        if #injured > 0 then
            ...
            if #injured <= Config.AlertShowInfo then
                for k, v in pairs(injured) do
                    limbDamageMsg = limbDamageMsg .. "Dine " .. v.label .. " føles "..Config.WoundStates[v.severity]
                    if k < #injured then
                        limbDamageMsg = limbDamageMsg .. " | "
                    end
                end
            else
                limbDamageMsg = "Du har smerter forskellige steder.."
            end
...
RegisterNetEvent('hospital:client:Revive', function()
    ...
    QBCore.Functions.Notify("Du er fuldstændig rask!")
end)
...
RegisterNetEvent('hospital:client:HealInjuries', function(type)
    ...
    TriggerServerEvent("hospital:server:RestoreWeaponDamage")
    QBCore.Functions.Notify("Alle dine kvæstelser er helet!")
end)
...
CreateThread(function()
    while true do
        ...
        if isInHospitalBed and canLeaveBed then
            sleep = 0
            local pos = GetEntityCoords(PlayerPedId())
            DrawText3D(pos.x, pos.y, pos.z, "~g~E~w~ - For at rejse dig fra sengen..")
        ...
        if LocalPlayer.state['isLoggedIn'] then
            ...
            for k, checkins in pairs(Config.Locations["checking"]) do
                if #(pos - checkins) < 1.5 then
                    ...
                    if doctorCount >= Config.MinimalDoctors then
                        DrawText3D(checkins.x, checkins.y, checkins.z, "~g~E~w~ - Kald på en læge")
                    else
                    ...
                    if IsControlJustReleased(0, 38) then
                        if doctorCount >= Config.MinimalDoctors then
                            ...
                            QBCore.Functions.Progressbar("hospital_checkin", "Check-in...", 2000, false, true, {
                                ...
                                if bedId then
                                    ...
                                    QBCore.Functions.Notify("Alle senge er optaget..", "error")
                                end
                            end, function() -- Cancel
                                TriggerEvent('animations:client:EmoteCommandStart', {"c"})
                                QBCore.Functions.Notify("Check-in fejlede!", "error")
                            end)
                        end
                    end
            ...
            if closestBed and not isInHospitalBed then
                if #(pos - vector3(Config.Locations["beds"][closestBed].coords.x, Config.Locations["beds"][closestBed].coords.y, Config.Locations["beds"][closestBed].coords.z)) < 2 then
                    sleep = 5
                    DrawText3D(Config.Locations["beds"][closestBed].coords.x, Config.Locations["beds"][closestBed].coords.y, Config.Locations["beds"][closestBed].coords.z + 0.3, "~g~E~w~ - For at lægge dig")
                    if IsControlJustReleased(0, 38) then
