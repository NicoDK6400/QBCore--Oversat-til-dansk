...
Citizen.CreateThread(function()
    while true do
        ...
        if distance1 < 15 then
            ...
            if distance1 < 2 then
                DrawText3Ds(509.89, 6478.76, 29.77, "[G] Start med at pluk")
                if IsControlJustPressed(0, 47) then
                    PrepareAnim()
                    PickMinigame()
                end
            ...
            if distance2 < 2 then
                DrawText3Ds(509.47, 6472.89, 29.77, "[G] Start med at pluk")
                if IsControlJustPressed(0, 47) then
                    PrepareAnim()
                    PickMinigame()
                end
            ...
            if distance3 < 2 then
                DrawText3Ds(509.47, 6467.10, 29.74, "[G] Start med at pluk")
                if IsControlJustPressed(0, 47) then
                    PrepareAnim()
                    PickMinigame()
                end
...
function pickProcess()
    QBCore.Functions.Progressbar("grind_coke", "Plukker kokain blade ..", math.random(80000,120000), false, true, {
...
function cokeProcess()
    QBCore.Functions.Progressbar("grind_coke", "Klargøre kokain blade ..", math.random(10000, 12000), false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
...
function crackProcess()
    QBCore.Functions.Progressbar("grind_coke", "Klargøre crack ..", math.random(10000, 12000), false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
...
function PickMinigame()
    ...
    Skillbar.Start({
        ...
        if SucceededAttempts + 1 >= NeededAttempts then
            pickProcess()
            QBCore.Functions.Notify("Du plukkede et kokainblad", "success")
            FailedAttemps = 0
            SucceededAttempts = 0
            NeededAttempts = 0
        else    
...
function ProcessMinigame(source)
    ...
    Skillbar.Start({
        ...
        if SucceededAttempts + 1 >= NeededAttempts then
            cokeProcess()
            QBCore.Functions.Notify("Du laver kokain!", "success")
            FailedAttemps = 0
            SucceededAttempts = 0
            NeededAttempts = 0
        else    
...
function ProcessCrackMinigame(source)
    ...
    Skillbar.Start({
        ...
        if SucceededAttempts + 1 >= NeededAttempts then
            crackProcess()
            QBCore.Functions.Notify("Du laver crack!", "success")
            FailedAttemps = 0
            SucceededAttempts = 0
            NeededAttempts = 0
        else    
...
function knockDoorAnim(home)
    ...
    if home then
        ...
        knockingDoor = false
        Citizen.Wait(1000)
        QBCore.Functions.Notify('Ser ud til at der ikke er nogen hjemme..', 'error', 3500)
    end
end
