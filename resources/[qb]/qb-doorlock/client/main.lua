...
local function lockpickFinish(success)
	if success then
		QBCore.Functions.Notify('Det lykkedes!', 'success', 2500)
		setDoorLocking(closestDoorValue, closestDoorKey)
	else
		QBCore.Functions.Notify('Det fejlede!', 'error', 2500)
	end
end
		...
		if dist < 1.5 then
			if QB.Doors[k].pickable then
				if QB.Doors[k].locked then
					...
					QBCore.Functions.Notify('Denne dør er ikke låst', 'error', 2500)
					break
				end
			else
