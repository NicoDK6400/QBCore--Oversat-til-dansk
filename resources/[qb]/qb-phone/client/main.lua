...
local function OpenPhone()
    QBCore.Functions.TriggerCallback('qb-phone:server:HasPhone', function(HasPhone)
        if HasPhone then
            ...
            QBCore.Functions.Notify("Du har ikke en mobil", "error")
        end
    end)
end
...
local function CancelCall()
    ...
    if not PhoneData.isOpen then
        SendNUIMessage({
            ...
            PhoneNotify = {
                title = "Mobil",
                text = "Telefonopkaldet er blevet afbrudt",
                icon = "fas fa-phone",
                color = "#e84118",
            },
        })
...
RegisterCommand('phone', function()
    ...
    if not PhoneData.isOpen and LocalPlayer.state.isLoggedIn then
        if not PlayerData.metadata['ishandcuffed'] and not PlayerData.metadata['inlaststand'] and not PlayerData.metadata['isdead'] and not IsPauseMenuActive() then
            ...
            QBCore.Functions.Notify("Handlingen er ikke muligt..", "error")
        end
    end
end)
...
RegisterNUICallback('track-vehicle', function(data, cb)
    ...
    if findVehFromPlateAndLocate(veh.plate) then
        QBCore.Functions.Notify("Dit køretøj er blevet markeret", "success")
    else
        ...
        if v.name == Name and v.number == Number then
            ...
            --if PhoneData.isOpen then
                SendNUIMessage({
                    ...
                    PhoneNotify = {
                        title = "Mobil",
                        text = "Du har slettet en kontakt!",
...
RegisterNUICallback('SetAlertWaypoint', function(data)
    ...
    QBCore.Functions.Notify('GPS lokation sat: '..data.alert.title)
...
RegisterNUICallback('RaceDistanceCheck', function(data, cb)
    QBCore.Functions.TriggerCallback('qb-lapraces:server:GetRacingData', function(RaceData)
        ...
        if dist <= 115.0 then
            ...
            QBCore.Functions.Notify('Du er for langt væk fra løbet. En GPS er sat!', 'error', 5000)
...
RegisterNUICallback('SetHouseLocation', function(data, cb)
    ...
    QBCore.Functions.Notify("GPS'en er sat til " .. data.HouseData.HouseData.adress .. "!", "success")
end)
...
RegisterNUICallback('SetGPSLocation', function(data, cb)
    ...
    QBCore.Functions.Notify("GPS'en er sat!", "success")
end)
    ...
    if (PlayerData.money.bank - amount) >= 0 then
        QBCore.Functions.TriggerCallback('qb-phone:server:CanTransferMoney', function(Transferd)
            if Transferd then
                ...
                SendNUIMessage({ action = "PhoneNotification", PhoneNotify = { timeout=3000, title = "Bank", text = "Kontoen eksistere ikke!", icon = "fas fa-university", color = "#ff0000", }, })
    ...
    if not delete then -- New Tweet
        if src ~= MyPlayerId then
            ...
            SendNUIMessage({
                ...
                PhoneNotify = {
                    title = "Twitter",
                    text = "Dit tweet er blevet posted!",
                    icon = "fab fa-twitter",
                    color = "#1DA1F2",
                    timeout = 1000,
                },
        ...
        if src == MyPlayerId then
            SendNUIMessage({
                ...
                PhoneNotify = {
                    title = "Twitter",
                    text = "Dit tweet er blevet slettet!",
                    icon = "fab fa-twitter",
                    color = "#1DA1F2",
                    timeout = 1000,
                },
...
RegisterNetEvent('qb-phone:client:UpdateAdverts', function(Adverts, LastAd)
    ...
    SendNUIMessage({
        ...
        PhoneNotify = {
            title = "Reklame",
            text = "En ny reklame er lagt op af "..LastAd,
            icon = "fas fa-ad",
            color = "#ff8f1a",
            timeout = 2500,
        },
...
RegisterNetEvent('qb-phone:client:CancelCall', function()
    ...
    if not PhoneData.isOpen then
        SendNUIMessage({
            ...
            NotifyData = {
                title = "Mobil",
                content = "Telefonopkaldet er blevet afbrudt",
                icon = "fas fa-phone",
                timeout = 3500,
                color = "#e84118",
            },
        ...
        if PhoneData.isOpen then
            if SenderNumber ~= PhoneData.PlayerData.charinfo.phone then
                ...
                SendNUIMessage({
                    ...
                    PhoneNotify = {
                        title = "Whatsapp",
                        text = "Besked sendt til dig selv",
                        icon = "fab fa-whatsapp",
                        color = "#25D366",
                        timeout = 4000,
                    },
...
RegisterNetEvent('qb-phone:client:AddNewSuggestion', function(SuggestionData)
    ...
    SendNUIMessage({
        ...
        PhoneNotify = {
            title = "Mobil",
            text = "Du har en ny forslået kontakt!",
            icon = "fa fa-phone-alt",
            color = "#04b543",
            timeout = 1500,
        },
