...
QBCore.Functions.CreateCallback('qb-phone:server:PayInvoice', function(source, cb, society, amount, invoiceId, sendercitizenid)
    ...
    if SenderPly and Config.BillingCommissions[society] then
        ...
        invoiceMailData = {
            ...
            message = string.format('Du har modtaget en provision af $%s når %s %s betaler regningen på %s DKK.', commission, Ply.PlayerData.charinfo.firstname, Ply.PlayerData.charinfo.lastname, amount)
        }
    elseif not SenderPly and Config.BillingCommissions[society] then
        invoiceMailData = {
            sender = 'Regnskabsafdelingen',
            subject = 'Regning betalt',
            message = string.format('%s %s betalte regningen på $%s', Ply.PlayerData.charinfo.firstname, Ply.PlayerData.charinfo.lastname, amount)
        }
    end
...
QBCore.Functions.CreateCallback("qb-phone:server:GetWebhook",function(source,cb)
	if WebHook ~= "" then
        ...
        print('Sæt dit webhook, ellers vil kamera ikke virke!!!!!! Den findes på linje 10 i server sided script!!!!!')
		cb(nil)
	end
...
RegisterNetEvent('qb-phone:server:TransferMoney', function(iban, amount)
    ...
    if result[1] ~= nil then
        ...
        TriggerClientEvent('QBCore:Notify', src, "Dette kontonummer eksistere ikke!", "error")
    end
end)
...
QBCore.Commands.Add('bill', 'Send regning', {{name = 'id', help = 'Spiller ID'}, {name = 'amount', help = 'Beløbet på regning'}}, false, function(source, args)
    ...
    if biller.PlayerData.job.name == "police" or biller.PlayerData.job.name == 'ambulance' or biller.PlayerData.job.name == 'mechanic' then
        if billed ~= nil then
            if biller.PlayerData.citizenid ~= billed.PlayerData.citizenid then
                ...
                TriggerClientEvent('QBCore:Notify', source, 'Du kan ikke give dig selv en regning', 'error')
            end
        else
