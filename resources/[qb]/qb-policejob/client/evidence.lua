...
StatusList = {
    ["fight"] = "Hævet næver",
    ["widepupils"] = "Store pupiller",
    ["redeyes"] = "Røde øjne",
    ["weedsmell"] = "Lugter af weed",
    ["gunpowder"] = "Krudtslam på tøjet",
    ["chemicals"] = "lugter af kemikalier",
    ["heavybreath"] = "Dybe indåndinger",
    ["sweat"] = "Sveder en del",
    ["handbleed"] = "Blod på hænderne",
    ["confused"] = "Forvirret",
    ["alcohol"] = "Lugter af alkohol",
    ["heavyalcohol"] = "Lugter kraftigt af alkohol"
}
...
AddEventHandler('evidence:client:SetStatus', function(statusId, time)
    if time > 0 and StatusList[statusId] ~= nil then
        if (CurrentStatusList == nil or CurrentStatusList[statusId] == nil) or
            (CurrentStatusList[statusId] ~= nil and CurrentStatusList[statusId].time < 20) then
...
AddEventHandler("evidence:client:ClearBlooddropsInArea", function()
    ...
    local blooddropList = {}
    QBCore.Functions.Progressbar("clear_blooddrops", "Opsamler blod..", 5000, false, true, {
        ...
        if Blooddrops ~= nil and next(Blooddrops) ~= nil then
            ...
            QBCore.Functions.Notify("Blod blev indsamlet :)")
        end
    end, function() -- Cancel
        QBCore.Functions.Notify("Blodet kunne ikke opsamles..", "error")
    end)
end)
...
AddEventHandler('evidence:client:AddCasing', function(casingId, weapon, coords, serie)
    Casings[casingId] = {
        type = weapon,
        serie = serie ~= nil and serie or "Serienummeret er ikke synligt..",
        coords = {
            x = coords.x,
            y = coords.y,
            z = coords.z - 0.9
...
AddEventHandler("evidence:client:ClearCasingsInArea", function()
    local pos = GetEntityCoords(PlayerPedId())
    local casingList = {}
    QBCore.Functions.Progressbar("clear_casings", "Fjernet patronhylster..", 5000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true
        ...
        if Casings ~= nil and next(Casings) ~= nil then
            ...
            QBCore.Functions.Notify("Kuglen blev fjernet fra hylstret :)")
        end
    end, function() -- Cancel
        QBCore.Functions.Notify("Kuglen blev ikke separeret", "error")
    end)
end)
...
Citizen.CreateThread(function() -- Gunpowder Status when shooting
    while true do
        ...
        if IsPedShooting(PlayerPedId()) or IsPedDoingDriveby(PlayerPedId()) then
            ...
            if weapon ~= GetHashKey("WEAPON_UNARMED") and weapon ~= GetHashKey("WEAPON_SNOWBALL") and weapon ~=
                GetHashKey("WEAPON_STUNGUN") and weapon ~= GetHashKey("WEAPON_PETROLCAN") and weapon ~=
...
Citizen.CreateThread(function()
    while true do
        ...
        if CurrentCasing ~= nil and CurrentCasing ~= 0 then
            local pos = GetEntityCoords(PlayerPedId())
            if #(pos -
                vector3(Casings[CurrentCasing].coords.x, Casings[CurrentCasing].coords.y,
                    Casings[CurrentCasing].coords.z)) < 1.5 then
                DrawText3D(Casings[CurrentCasing].coords.x, Casings[CurrentCasing].coords.y,
                    Casings[CurrentCasing].coords.z, "~g~G~w~ - Patron hylster ~b~#" .. Casings[CurrentCasing].type)
                if IsControlJustReleased(0, 47) then
                    local s1, s2 = Citizen.InvokeNative(0x2EB41072B4C1E4C0, Casings[CurrentCasing].coords.x,
                        Casings[CurrentCasing].coords.y, Casings[CurrentCasing].coords.z, Citizen.PointerValueInt(),
        ...
        if CurrentBlooddrop ~= nil and CurrentBlooddrop ~= 0 then
            ...
            if #(pos - vector3(Blooddrops[CurrentBlooddrop].coords.x, Blooddrops[CurrentBlooddrop].coords.y,
                ...
                DrawText3D(Blooddrops[CurrentBlooddrop].coords.x, Blooddrops[CurrentBlooddrop].coords.y,
                    Blooddrops[CurrentBlooddrop].coords.z,
                    "~g~G~w~ - Blod ~b~#" .. DnaHash(Blooddrops[CurrentBlooddrop].citizenid))
                if IsControlJustReleased(0, 47) then
                    local s1, s2 = Citizen.InvokeNative(0x2EB41072B4C1E4C0, Blooddrops[CurrentBlooddrop].coords.x,
                        Blooddrops[CurrentBlooddrop].coords.y, Blooddrops[CurrentBlooddrop].coords.z,
        ...
        if CurrentFingerprint ~= nil and CurrentFingerprint ~= 0 then
            ...
            if #(pos - vector3(Fingerprints[CurrentFingerprint].coords.x, Fingerprints[CurrentFingerprint].coords.y,
                ...
                DrawText3D(Fingerprints[CurrentFingerprint].coords.x, Fingerprints[CurrentFingerprint].coords.y,
                    Fingerprints[CurrentFingerprint].coords.z, "~g~G~w~ - Fingeraftryk ")
                if IsControlJustReleased(0, 47) then
                    local s1, s2 = Citizen.InvokeNative(0x2EB41072B4C1E4C0, Fingerprints[CurrentFingerprint].coords.x,
                        Fingerprints[CurrentFingerprint].coords.y, Fingerprints[CurrentFingerprint].coords.z,
                        Citizen.PointerValueInt(), Citizen.PointerValueInt())
                    local street1 = GetStreetNameFromHashKey(s1)
                    local street2 = GetStreetNameFromHashKey(s2)
                    local streetLabel = street1
    ...
    while true do
        ...
        if isLoggedIn then
            if PlayerJob.name == "police" and onDuty then
                if IsPlayerFreeAiming(PlayerId()) and GetSelectedPedWeapon(PlayerPedId()) ==
                    GetHashKey("WEAPON_FLASHLIGHT") then
            ...
            if IsPlayerFreeAiming(PlayerId()) and GetSelectedPedWeapon(PlayerPedId()) == GetHashKey("WEAPON_FLASHLIGHT") then
                if PlayerJob.name == "police" and onDuty then
                    for k, v in pairs(BlooddropsNear) do
                        if v ~= nil then
                            DrawMarker(27, v.coords.x, v.coords.y, v.coords.z - 0.05, 0.0, 0.0, 0.0, 180.0, 0.0, 0.0,
                                0.11, 0.11, 0.3, 250, 0, 50, 255, false, true, 2, false, false, false, false)
                        end
                    end
                end
            else
            ...
            if IsPlayerFreeAiming(PlayerId()) and GetSelectedPedWeapon(PlayerPedId()) == GetHashKey("WEAPON_FLASHLIGHT") then
                if PlayerJob.name == "police" and onDuty then
                    for k, v in pairs(CasingsNear) do
                        if v ~= nil then
                            DrawMarker(27, v.coords.x, v.coords.y, v.coords.z - 0.05, 0.0, 0.0, 0.0, 180.0, 0.0, 0.0,
                                0.11, 0.11, 0.3, 50, 0, 250, 255, false, true, 2, false, false, false, false)
                        end
                    end
                end
            else
            ...
            if IsPlayerFreeAiming(PlayerId()) and GetSelectedPedWeapon(PlayerPedId()) == GetHashKey("WEAPON_FLASHLIGHT") then
                if PlayerJob.name == "police" and onDuty then
                    for k, v in pairs(FingerprintsNear) do
                        if v ~= nil then
                            DrawMarker(27, v.coords.x, v.coords.y, v.coords.z - 0.05, 0.0, 0.0, 0.0, 180.0, 0.0, 0.0,
                                0.11, 0.11, 0.3, 23, 173, 12, 255, false, true, 2, false, false, false, false)
                        end
                    end
                end
            else
...
local SilentWeapons = {"WEAPON_PETROLCAN", "WEAPON_STUNGUN", "WEAPON_FIREEXTINGUISHER", "WEAPON_UNARMED"}
Citizen.CreateThread(function()
        ...
        if isArmed then
            if PlayerJob.name ~= "police" then
                if IsPedShooting(PlayerPedId()) and not IsSilentWeapon(currentWeapon) and IsPedNearby() then
                    ...
                    local s1, s2 = Citizen.InvokeNative(0x2EB41072B4C1E4C0, coords.x, coords.y, coords.z,
                        Citizen.PointerValueInt(), Citizen.PointerValueInt())
                    ...
                    if IsPedInAnyVehicle(PlayerPedId(), true) then
                        ...
                        TriggerServerEvent("police:server:GunshotAlert", streetLabel, automatic, true, coords,
                            vehicleInfo)
                    else
