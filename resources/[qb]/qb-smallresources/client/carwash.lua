...
RegisterNetEvent('qb-carwash:client:washCar', function()
    ...
    washingVehicle = true
    QBCore.Functions.Progressbar("search_cabin", "Køretøjet bliver vakset...", math.random(4000, 8000), false, true, {
        ...
        QBCore.Functions.Notify("Vask afbrudt", "error")
        washingVehicle = false
    end)
end)
                    ...
                    if dist <= 7.5 then
                        if Driver == PlayerPed then
                            if not washingVehicle then
                                ...
                                DrawText3Ds(Config.CarWash[k]["coords"]["x"], Config.CarWash[k]["coords"]["y"], Config.CarWash[k]["coords"]["z"], 'Vaskehallen er i brug..')
                            end
                        end
                    end
                end
