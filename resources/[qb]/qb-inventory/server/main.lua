...
local function CreateNewDrop(source, fromSlot, toSlot, itemAmount)
	...
	if Player.Functions.RemoveItem(itemData.name, itemAmount, itemData.slot) then
		...
		TriggerClientEvent("QBCore:Notify", source, "Du har ikke den genstand!", "error")
		return
	end
end
...
RegisterNetEvent('inventory:server:OpenInventory', function(name, id, other)
	...
	if not ply.state.inv_busy then
		if name ~= nil and id ~= nil then
			...
			elseif name == "glovebox" then
				...
				if Gloveboxes[id] ~= nil and Gloveboxes[id].isOpen then
					secondInv.name = "none-inv"
					secondInv.label = "Handskerum-intet"
					secondInv.maxweight = 10000
					secondInv.inventory = {}
					secondInv.slots = 0
				else
			...
			elseif name == "otherplayer" then
				...
				if Drops[id] ~= nil and not Drops[id].isOpen then
					secondInv.name = id
					secondInv.label = "Smidt-"..tostring(id)
		...
		TriggerClientEvent('QBCore:Notify', src, 'Ingen adgang', 'error')
	end
end)
...
RegisterNetEvent('inventory:server:SetInventoryData', function(fromInventory, toInventory, fromSlot, toSlot, fromAmount, toAmount)
		...
		if fromItemData ~= nil and fromItemData.amount >= fromAmount then
			...
			elseif QBCore.Shared.SplitStr(toInventory, "-")[1] == "traphouse" then
				...
				if IsItemValid then
					...
					TriggerClientEvent('QBCore:Notify', src, "Du kan ikke sælge denne genstand..", 'error')
				end
			else
				-- drop
			...
			TriggerClientEvent("QBCore:Notify", src, "Du har ikke denne genstand!", "error")
		end
	elseif QBCore.Shared.SplitStr(fromInventory, "-")[1] == "otherplayer" then
		...
		if fromItemData ~= nil and fromItemData.amount >= fromAmount then
			...
			TriggerClientEvent("QBCore:Notify", src, "Genstand eksistere ikke??", "error")
		end
	...
	elseif fromInventory == "crafting" then
		...
		if hasCraftItems(src, itemData.costs, fromAmount) then
			...
			TriggerClientEvent('QBCore:Notify', src, "Du har ikke de rigtige genstande..", "error")
		end
	elseif fromInventory == "attachment_crafting" then
		...
		if fromItemData ~= nil and fromItemData.amount >= fromAmount then
			...
			TriggerClientEvent("QBCore:Notify", src, "Denne genstand eksistere ikke??", "error")
		end
	end
end)
    ...
    if dist < 2 then
        if amount <= item.amount then
            ...
            if OtherPlayer.Functions.AddItem(item.name, amount, false, item.info) then
                ...
                TriggerClientEvent('QBCore:Notify', target, "Du modtog "..amount..' '..item.label.." fra "..Player.PlayerData.charinfo.firstname.." "..Player.PlayerData.charinfo.lastname)
                ...
                TriggerClientEvent('QBCore:Notify', src,  "Modtageren har ikke nok plads!", "error")
                TriggerClientEvent('QBCore:Notify', target,  "Dit inventory er fyldt!", "error")
				TriggerClientEvent("inventory:client:UpdatePlayerInventory", src, false)
				TriggerClientEvent("inventory:client:UpdatePlayerInventory", target, false)
            end
        else
            TriggerClientEvent('QBCore:Notify', src, "Du har ikke nok at give af")
        end
    else
        TriggerClientEvent('QBCore:Notify', src, "Du er for langt væk!")
    end
end)
...
QBCore.Commands.Add("resetinv", "Reset Inventory (Kun Admin)", {{name="type", help="stash/trunk/glovebox"},{name="id/plate", help="ID af stash eller reg.nr."}}, true, function(source, args)
	...
	if invType ~= nil and invId ~= nil then
		...
		TriggerClientEvent('QBCore:Notify', source,  "Argumenter er ikke udfyldt korrekt..", "error")
	end
end, "admin")
		...
		if amount > 0 then
			if itemData ~= nil then
				...
				if Player.Functions.AddItem(itemData["name"], amount, false, info) then
					...
					TriggerClientEvent('QBCore:Notify', source,  "Du kan ikke give ting!", "error")
				end
			else
				TriggerClientEvent('QBCore:Notify', source,  "Enheden eksistere ikke", "error")
			end
		else
			TriggerClientEvent('QBCore:Notify', source,  "Ugylig mængde", "error")
		end
	else
...
QBCore.Commands.Add("randomitems", "Giv random ting (Kun God)", {}, false, function(source, args)
	local Player = QBCore.Functions.GetPlayer(source)
	local filteredItems = {}
