...
QBCore.Commands.Add("setcryptoworth", "Sæt krypto valuta værdi", {{name="crypto", help="Navnet på krypto valutaen"}, {name="Value", help="Ny kurs på krypto valutaen"}}, false, function(source, args)
    local src = source
    ...
    if crypto ~= nil then
        if Crypto.Worth[crypto] ~= nil then
            ...
            TriggerClientEvent('QBCore:Notify', src, "Denne krypto valuta findes ikke :(, ledigt: Qbit")
        end
    else
        TriggerClientEvent('QBCore:Notify', src, "Du har ikke oplyst krypto, ledigt er: Qbit")
    end
end, "admin")

QBCore.Commands.Add("checkcryptoworth", "", {}, false, function(source)
    local src = source
    TriggerClientEvent('QBCore:Notify', src, "Qbit har en værdi af: "..Crypto.Worth["qbit"].. " DKK")
end)
...
RegisterServerEvent('qb-crypto:server:ExchangeFail', function()
    ...
    if ItemData ~= nil then
        ...
        TriggerClientEvent('QBCore:Notify', src, "Krypto usb'en virker ikke", 'error')
    end
end)
