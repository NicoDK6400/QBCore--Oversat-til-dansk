    ...
    if Inventory[CurrentWeaponData.slot] then
        if Inventory[CurrentWeaponData.slot].info.attachments and next(Inventory[CurrentWeaponData.slot].info.attachments) then
            ...
            if not HasAttach then
                ...
                TriggerClientEvent("QBCore:Notify", src, "Du har allerede "..AttachmentData.label:lower().." på dit våben.", "error", 3500)
            end
        else
...
QBCore.Commands.Add("repairweapon", "Reparer våben (Kun God)", {{name="hp", help="HP på dit våben"}}, true, function(source, args)
