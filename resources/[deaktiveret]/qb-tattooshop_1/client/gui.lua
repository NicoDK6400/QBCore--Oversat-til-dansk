...
local function debugPrint(text)
    if TattooMenu.debug then
        Citizen.Trace('[TattooMenu] '..tostring(text))
    end
end
...
-- API
function TattooMenu.CreateMenu(id, title, closeCallback)
    -- Default settings
    menus[id] = { }
...
function TattooMenu.CreateSubMenu(id, parent, subTitle, closeCallback)
    if menus[parent] then
        TattooMenu.CreateMenu(id, menus[parent].title)
...
function TattooMenu.CurrentMenu()
    return currentMenu
end

function TattooMenu.MenuTable()
    return menus
end


function TattooMenu.OpenMenu(id)
    if id and menus[id] then
...
function TattooMenu.IsMenuOpened(id)
    return isMenuVisible(id)
end


function TattooMenu.IsAnyMenuOpened()
...
function TattooMenu.IsAnyMenuWithTitleOpened(subtitle)
...
function TattooMenu.IsMenuAboutToBeClosed()
    if menus[currentMenu] then
...
function TattooMenu.IsThisMenuAboutToBeClosed(id)
    if menus[id] then
...
function TattooMenu.Button(text, subText)
...
function TattooMenu.DisabledButton(text, subText)
    local buttonText = text
    if subText then
        buttonText = '{ '..tostring(buttonText)..', '..tostring(subText)..' }'
    end
...
function TattooMenu.SpriteButton(text, textDict, sprite, focusSprite)
    local buttonText = text
...
function TattooMenu.SpriteMenuButton(text, textDict, sprite, focusSprite, id)
    if menus[id] then
        local clicked, hovered = TattooMenu.SpriteButton(text, textDict, sprite, focusSprite)
        if clicked then
            setMenuVisible(currentMenu, false)
...
function TattooMenu.ComboBox(text, items, currentIndex, selectedIndex, callback, displaycb)
    ...
    if TattooMenu.Button(text, selectedItem) then
...
function TattooMenu.MenuButton(text, id, secondtext)
    if menus[id] then
		local clicked, hovered = TattooMenu.Button(text, (secondtext and secondtext or "→")) 
		if clicked then
            setMenuVisible(currentMenu, false)
            setMenuVisible(id, true, true)
...
function TattooMenu.SwitchMenu(id)
	setMenuVisible(currentMenu, false)
    setMenuVisible(id, true, true)
end

function TattooMenu.SetDescription(text)
	currentDesc = tostring(text)
end

function TattooMenu.CheckBox(text, bool, callback)
    --[[local checked = '~r~Off'
    if bool then
        checked = '~g~On'
    end]]
    ...
    if TattooMenu.SpriteButton(text, "commonmenu", sprite, focusSprite) then
        bool = not bool
...
function TattooMenu.SetMenuWidth(id, width)
    setMenuProperty(id, 'width', width)
end


function TattooMenu.SetMenuX(id, x)
    setMenuProperty(id, 'x', x)
end


function TattooMenu.SetMenuY(id, y)
    setMenuProperty(id, 'y', y)
end


function TattooMenu.SetMenuMaxOptionCountOnScreen(id, count)
    setMenuProperty(id, 'maxOptionCount', count)
end


function TattooMenu.SetTitleColor(id, r, g, b, a)
    setMenuProperty(id, 'titleColor', { r, g, b, a or  menus[id].titleColor[4] })
end


function TattooMenu.SetTitleBackgroundColor(id, r, g, b, a)
    setMenuProperty(id, 'titleBackgroundColor', { r, g, b, a or menus[id].titleBackgroundColor[4] })
end


function TattooMenu.UseSpriteAsBackground(id, textDict, sprite, r, g, b, a, stillDrawText)
