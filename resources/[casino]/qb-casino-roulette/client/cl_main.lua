            ...
            Citizen.CreateThread(function()
                while selectedRulett ~= nil do
                    ...
                    if IsDisabledControlPressed(0, 172) then
                        ...
                        QBCore.Functions.Notify('+DKK'..currentBetAmount.." bet [hævet]",'success')
                    ...
                    elseif IsDisabledControlPressed(0, 173) then
                        if currentBetAmount > 0 then
                            ...
                            changeBetAmount(currentBetAmount)
                            QBCore.Functions.Notify('-DKK'..currentBetAmount.." bet [nedsat]",'primary')
            ...
            Citizen.CreateThread(function()
                while selectedRulett ~= nil do
                    ...
                    if IsDisabledControlJustPressed(0, 22) then --Custom Bet [space]
                        local tmpInput = getGenericTextInput('Hvor meget vil du bet?')
                        if tonumber(tmpInput) then
                            tmpInput = tonumber(tmpInput)
                            if tmpInput > 0 then
                                changeBetAmount(tmpInput)
                                QBCore.Functions.Notify('Indtast bet '..currentBetAmount,'success')
                                        ...
                                        if currentBetAmount > 0 then
                                            if Config.RulettTables[selectedRulett] ~= nil then
                                                if currentBetAmount >= Config.RulettTables[selectedRulett].minBet and currentBetAmount <= Config.RulettTables[selectedRulett].maxBet then
                                                    ...
                                                    QBCore.Functions.Notify('Dit bet er enten for lavt eller for højt til dette bord.','error')

                                                end
                                            end
                                        else
                                            QBCore.Functions.Notify('Ugyldig værdi.','error')
                                        end
                                    end
                                end
                            end
        ...
        if closetoRulett and selectedRulett == nil then
            for k, v in pairs(Rulettek) do
                if DoesEntityExist(v.tableObject) then
                    ...
                    if dist < 2.4 then
                        if dist < 2.3 then
                            exports['textUi']:DrawTextUi('show',"Diamond Casino Roulette</p>Tryk [E] for at tage plads")
                            ...
                            if IsControlJustPressed(0, 38) then
                                QBCore.Functions.TriggerCallback('QBCore:HasItem', function(HasItem)
                                    if HasItem then
							            ...
							            QBCore.Functions.Notify('Du er ikke et medlem af dette casino', 'error', 3500)
						            end
					            end, 'member')
                            end
...
function casinoNuiUpdateGame(rulettIndex, ido, statusz)
    if selectedRulett == rulettIndex then
        ...
        if not statusz then
			...
			exports['casinoUi']:DrawCasinoUi('show', "Diamond Casino Blackjack</p>"..ido.." Sekunder tilbage</p>Nuværende bet: "..currentBetAmount.." DKK</p>Balance: "..bankAmount.." DKK")
            exports['textUi']:DrawTextUi('show', "Juster bet: <strong>↑/↓</strong></p>VENSTRE KLIK: Bet nummer</p>SPACEBAR: Selvvalgt bet</p>E: Ændre kamera vinkel</p>ESC: Forlad")
        else
...
AddEventHandler(
    ...
    function(chairId)
        QBCore.Functions.Notify('Du tabte... Bedre held næste gang', 'error', 3500)
